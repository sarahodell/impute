...
for(h in 1:length(hap)){
   ...
   if(is.null(dim(hsub))==F){
	...
	saveRDS(dropped,sprintf('bg%s_haplogroup%0.f_dropped_markers.rds',c,h))
	filtered_hap=c()
	for(k in 1:h){filtered_hap[[k]]=regrp[[k]][,keep]}
	saveRDS(filtered_hap,sprintf('bg%s_filtered_haplogroup%.0f_probs_%s.rds',chr,h,date))
